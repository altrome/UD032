...
def shape_element(element):
    ...
    if element.tag == "node" or element.tag == "way" :
        ...
        #try if visible exists in tag
        try:
            node["visible"] = element.attrib["visible"]
        except KeyError:
            node["visible"] = "false"
        #try if lat & lon exists in tag
        try:
        ...
        if len(tags) > 0: #there are tags labelled "tag" inside the node
            for tag in tags:
                ...
                tagKeySplit = re.split('(\W+)', tagKey) #split all the words into tagKeySplit List
                if not problemchars.search(tagKey): # search for Problematic Chars
                    if not lower_colon.search(tagKey): #Not Matches xxxxxx:yyyyyyy       
                        if len(tagKeySplit) < 4: #less than 4 elements in tagKeySplit (xxx:yyy:zzz wont pass)
                            node[tagKey] = tagValue
                    else: #Matches xxxxxx:yyyyyyy 
                        if tagKeySplit[0] == "addr": #first word is "addr"
                            address[tagKeySplit[2]] = tagValue
                        else:
                            node[tagKey] = tagValue
...
def test():
    ...
    data = process_map('Improving_Street_Names.xml', True)
    #pprint.pprint(data)
