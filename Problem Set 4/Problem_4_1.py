            ...
            for field in process_fields:
                if line[field] == "NULL":
                    if field != 'name':
                        ...
                        line[field] = removeExtraInfo(line['rdf-schema#label'])
                else:
                    if field == 'rdf-schema#label':
                        line[field] = removeExtraInfo(line[field])
                    elif field == 'synonym':
                        line[field] = parse_array(line[field])
                    elif (field == 'class_label' or field == 'family_label' or field == 'order_label' or field == 'phylum_label') and line[field][0] == "{":
                        line[field] = parse_array(line[field])
                if field == 'rdf-schema#label' or field == 'URI' or field == 'rdf-schema#comment' or field == 'synonym' or field == 'name':
                    doc[fields[field]] = line[field]
                else:
                    classification[fields[field]] =line[field]
...
def parse_array(v):
    if (v[0] == "{") and (v[-1] == "}"):
        ...
        #remove "*"
        for key, item in enumerate(v_array):
            if item[0] == "*":
                v_array[key] = item[1:]
        v_array = [i.strip() for i in v_array]
        return v_array
